...
class ThreatInjection(object):
    ...
    def overwrite(self):
        ...
        if self.threat.active:
            ...
            if thr_name.endswith('_u'):
                act_name = thr_name.split('_u')[0]+'_activate'
            elif thr_name.endswith('_p'):
                act_name = thr_name.split('_p')[0]+'_activate'     
            else:
                ValueError("The name of threatened variables must ends with '_u' or '_p'!")
    ...
    def generate_signal(self):
        ...
        elif self.threat.temporal.attacker.upper() == 'MIN':
            t, signal = self.min_attack()
        elif self.threat.temporal.attacker.upper() == 'EXTERNAL':
            t, signal = self.external_attack()
        else:
            pass
        return t, signal
    ...
    def min_attack(self):
        """
        A model that launches the minimum attack on a signal.
        ...
        self.signal = np.array([self._min() for i in self.t]) 
    ...
    def external_attack(self):
        ...
        if 'external' not in kwargs:
            raise ValueError("Parameter 'external' of the external attacker has not been assigned!")
        elif type(kwargs['external']) is not tuple:
            raise ValueError("Parameter 'external' of the external attacker has to be a tuple in the format: (timestamp,signal)!")
        
        # assign time stamp
        self.t = self._timestamp()
        # get external signals that are within current time period

        return self._external(self.t, **kwargs)
    ...
    def _min(self):
        """
        Get the minimum allowable value of a signal

        :return: minimum value of a signal
        :rtype: float
        """        
        return self.threat.temporal.signal_type.min
    ...
    def _external(self,t, **kwargs):
        """
        Get external signal.
        Here we assume the external signal are step post signal.
        ...
        """
        t_ext = np.array(kwargs['external'][0])
        v_ext = np.array(kwargs['external'][1])

        ts = t[0]
        te = t[-1]

        # formulate new step signal between ts and te
        t_new = []
        v_new = []
        for i, ti in enumerate(t_ext):
            if ti>=ts and ti<=te:
                t_new += [ti]
                v_new += [v_ext[i]]
        # ts, te should be in the new list
        if ts not in t_new:
            # add ts to the first element
            t_new.reverse()
            t_new.append(ts)
            t_new.reverse()

            # add corresponding value for ts assuming post step
            vs = np.array(v_ext)[np.array(t_ext)<ts][-1]
            v_new.reverse()
            v_new.append(vs)
            v_new.reverse()
            
        if te not in t_new:
            # add te to the last element
            t_new.append(te)
            # add corresponding value for te
            ve = np.array(v_ext)[np.array(t_ext)<te][-1]
            v_new.append(ve)
        
        # need generate signal based on giving time stamp
        v=np.zeros(len(t))
        for i, ti in enumerate(np.array(t)):
            v[i] = np.array(v_new)[np.array(t_new)<=ti][-1]
    ...
    def _square(self,t, **kwargs):
        ...
        phase_array += phase
        wave = amplitude*(SG.square(phase_array,duty)+1.)/2. + offset
