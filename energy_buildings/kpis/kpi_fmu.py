'''This module contains the calculation of real time KPI at the end of each time step
and long-term KPI at the end of simulation based on FMU simulation results.
...
class KPI_Calculator(object):
    ...
    def __init__(self, kpi_names=[], 
        ...
        Parameters
        ----------
        data_buff: the result object after fmu simulation
        For example, data_buff=fmu.simulate()

        kpi_names: the names of KPIs to be calculated which should be part of the sources.
        ...
        with open(dir_path+"/kpis.json", "r") as json_data:
            self.kpi_json = json.loads(json_data.read())
        # check inputs
        self._check_kpi_names(kpi_names)
        
        #Occupancy start hour and end hour
        self.start_occ = start_occ
        self.end_occ = end_occ

        # key measurements specified by users. Used for debugging purpose
        self._measurement_names = measurement_names
        self.filter = self._get_filtered_variables()

    def _check_kpi_names(self,kpi_names):
        """Check if the user-specified kpi names are in the sources.
        """
        if len(kpi_names)>0:
            self.kpi_names = kpi_names
            for name in kpi_names:
                if name not in self.sources:
                    raise ValueError("KPI names not supported: "+name)
        else:
            self.kpi_names = self.sources # defaulted sources are used as kpi 

    def _update_kpi_measurement_names(self):
        """This method is to update kpi_json dictionary in the memory by using the updated measurement names.
        
        Returns
        -------
        An updated dictionay with only to-be calculated KPI names following the naming conventions of Modelica/FMU template
        """
        # update kpi json
        for key in self.kpi_json.keys():
            for i, signal in zip(range(len(self.kpi_json[key])),self.kpi_json[key]):
                self.kpi_json[key][i] = "mod."+signal
        

        # update user-specified measurement_names
        if self._measurement_names:
            for i, ele in zip(range(len(self._measurement_names)),self._measurement_names):
                self._measurement_names[i] = 'mod.'+ ele

    def _get_kpi_measurement_names(self):
        self._update_kpi_measurement_names()
        kpi_measurement_names = []

        for name in self.kpi_names:
            for key in self.kpi_json.keys():
                if key.startswith(name):
                    for signal in self.kpi_json[key]:
                        kpi_measurement_names += [signal]
        
        return kpi_measurement_names

    def _get_filtered_variables(self):

        """Get a list of filtered names of outputs. 
        The list should be added to fmu simulation options.

        """
        self.filter = self._measurement_names + self._get_kpi_measurement_names()
    ...
    def get_core_kpis(self, price_scenario='Constant'):
        ...
        ckpi['ener_tot'] = self.get_energy(plot=False) 
            ...
            if source.startswith('AirZoneTemperature') or \
               source.startswith('OperativeZoneTemperature'):
                ...
                for signal in self.kpi_json[source]:
                    ...
                    if len(index_Occ)>1: # if not empty                   
                        ...
                        tdis_dict[signal[:-1]+'dTlower_y'] = 0
                        tdis_dict[signal[:-1]+'dTupper_y'] = 0
               ...
               source.startswith('OperativeZoneTemperature'):
                ...
                for signal in self.kpi_json[source]:
                    ...
                    if len(index_Occ)>1: # if not empty   
                        ...
                        dT_max = 0
                        tim_dT_max = [int(index[0])]

                    #Tuple of (maximum temperature deviation, occur time) - this is discarded
                    # Instead of using tupel, we should use dictionary to be consistent with other KPIs in order to future compatibility.
                    #dT_max_dict[signal[:-1]+'[dT_max,tim_dT_max]']=(dT_max,list(set(tim_dT_max)))
                    dT_max_dict[signal[:-1]+'_dT_max']=dT_max
                    dT_max_dict[signal[:-1]+'_tim_dT_max'] = list(set(tim_dT_max))
    ...
    def get_unmet_hour(self, plot=False):
            ...
            if source.startswith('AirZoneTemperature') or \
               source.startswith('OperativeZoneTemperature'):
                ...
                for signal in self.kpi_json[source]:
                    ...
                    if len(index_Occ)>1: # if not empty
                        index_Occ_new=np.arange(index_Occ[0], index_Occ[-1], 60)
        ...
        self.unmet_fraction=round(tim_unmet_tot/max(tim_Occ_tot,1e-08),3)
        ...
        for source in self.sources:
            ...
            source in self.kpi_json.keys():            
                for signal in self.kpi_json[source]:
                    ...
                    ener_dict[signal+'_y'] = \
                        trapz(pow_data,
                              index)*2.77778e-7 # Convert to kWh
    ...
    def get_power_peaks(self):
        ...
        max_pow_tot=power_data_tot.max()/1000. # to kW
    ...
    def get_equiment_operation_time(self, plot=False):
        ...
        for signal in self.kpi_json['EquipmentOnOff']:
            equ_onoff_data = np.array(np.ceil(self.data_buff[signal]))
            tim_equ_ope_dict[signal+'_y'] = \
                trapz(equ_onoff_data,index)/3600.
    ...
    def get_oar_hourly(self):
        ...
        vot = np.array(self.data_buff['conAHU.eco.VOutMinSet_flow_normalized'])
        voa = np.array(self.data_buff['conAHU.eco.VOut_flow_normalized'])
    ...
    def get_control_quality_factors(self):
        ...
        for signal,setpoint in zip(self.kpi_json['ControlOutput'],self.kpi_json['ControlSetpoint']):
            if signal.split(".")[-1].startswith('T'):
                ControType='Temp'
                con = np.array(self.data_buff[signal])-273.15
                conSet = np.array(self.data_buff[setpoint])-273.15  
...
if __name__ == "__main__":
    ...
    kpical = KPI_Calculator(data_buff=None,
        kpi_names=['ElectricPower','AirZoneTemperature'], 
        measurement_names=[], 
        start_occ=6, 
        end_occ=19)
