...
# construct F using FMU: this is supposed to replace F
def FFMU(model,start_time,start_states,x):
    """

    x: design variables; 2-by-1
    """
    tf=x[-1]
    ut=x[0]
    end_time = start_time + dt
    model.set_time(start_time)
    model.set_states(start_states)
    model.set('acceleration',ut)
    res=model.simulate(start_time,end_time)
    end_states = model.get_states()
    
    return model, end_time, end_starts, res
