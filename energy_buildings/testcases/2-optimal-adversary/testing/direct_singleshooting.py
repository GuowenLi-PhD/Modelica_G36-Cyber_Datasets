...
# Formulate the NLP
Xk = MX([0, 1]) # initialize states at time=0
...
for k in range(N):
    ...
    Fk = F(x0=Xk, p=Uk) # get system states and systme outputs at the end of current step
    Xk = Fk['xf'] # get states
    J=J+Fk['qf'] # get outputs and overall objective till time step k

    # Add inequality constraint
    g += [Xk[1]] # constraints on x2 at each step
    lbg += [-.25]
    ubg += [inf]
