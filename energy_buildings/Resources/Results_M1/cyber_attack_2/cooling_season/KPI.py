...
fmu_attack = load_fmu("SignalBlocking.fmu")
...
TCHWSet_orig_attack = res_attack['temDifPreRes.TSet']
TCHWSet_corr_attack = res_attack['chiWSE.TSet']
...
plt.plot(np.array(t_attack),np.array(TCHWSet_corr_attack)-273.15,'r-', linewidth=1) # 
plt.plot(np.array(t_attack),np.array(TCHWSet_orig_attack)-273.15,'g:', linewidth=1) # 
...
plt.plot(np.array(t_base),np.array(qCooCoi_base)/1000,'b--',markevery=0.1,marker="o",markersize=3,linewidth=1) 
plt.plot(np.array(t_attack),np.array(qCooCoi_attack)/1000,'r-', linewidth=1) #
plt.ylabel('Heat Flow in Cooling\nCoil [kW]')
plt.ylim([-10,100])
plt.legend(['Baseline','Attack'])
plt.xticks(np.arange(startTime,endTime,4*3600),np.arange(0,25,4))
plt.xlabel('Hour')
...
chiSwiTime_base = res_base['swiTimChi.y'][-1] # to hours
boiSwiTime_base = res_base['swiTimBoi.y'][-1]
supFanSwiTime_base = res_base['swiTimFan.y'][-1]
cooTowSwiTime_base = res_base['swiTimCooTow.y'][-1]
CHWPSwiTime_base = res_base['swiTimCHWP.y'][-1]
CWPSwiTime_base = res_base['swiTimCWP.y'][-1]
HWPSwiTime_base = res_base['swiTimHWP.y'][-1]
...
chiSwiTime_attack = res_attack['swiTimChi.y'][-1]
boiSwiTime_attack = res_attack['swiTimBoi.y'][-1]
supFanSwiTime_attack = res_attack['swiTimFan.y'][-1]
cooTowSwiTime_attack = res_attack['swiTimCooTow.y'][-1]
CHWPSwiTime_attack = res_attack['swiTimCHWP.y'][-1]
CWPSwiTime_attack = res_attack['swiTimCWP.y'][-1]
HWPSwiTime_attack = res_attack['swiTimHWP.y'][-1]
