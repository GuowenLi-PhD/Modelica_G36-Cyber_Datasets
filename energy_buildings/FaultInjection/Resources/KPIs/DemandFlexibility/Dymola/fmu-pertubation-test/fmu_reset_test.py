...
while ts < endTime:
    ...
    TCooOn.extend(res_step_bas['TCooOn'])
    TZonCor.extend(res_step_bas['conVAVCor.TZon'])
    tim.extend(res_step_bas['time'])  
...
fig = plt.figure()
fig.add_subplot(211)
plt.plot(np.array(tim),np.array(TCooOn)-273.15,'k-', linewidth=1) # kW
plt.plot(np.array(tim),np.array(TZonCor)-273.15,'k--', linewidth=1) # kW
fig.add_subplot(212)
...
results=pd.DataFrame({'base':[(np.array(tim),np.array(TCooOn))],
                    'again':[(np.array(tim_again),np.array(TCooOn_again))]})

# dump as a json file
results.to_json('test_result.json')