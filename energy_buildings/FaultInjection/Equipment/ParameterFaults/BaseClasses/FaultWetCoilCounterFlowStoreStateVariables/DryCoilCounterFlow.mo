...
model DryCoilCounterFlow
  ...
  Modelica.SIunits.Temperature T_m[nEle] = ele[:].con1.solid.T
    "Metal temperature";
  Modelica.SIunits.TemperatureSlope der_T_m[nEle] = der(ele[:].con1.solid.T)
    "Metal temperature";
...
protected
  ...
  replaceable model HexElement =
      FaultInjection.Equipment.ParameterFaults.BaseClasses.FaultWetCoilCounterFlowStoreStateVariables.HexElementSensible
  ...
  HexElement ele[nEle](
    ...
    T1_start={iniT1[i] for i in 1:nEle},
    T2_start={iniT2[i] for i in 1:nEle})
