within FaultInjection.Equipment.ParameterFaults.BaseClasses.FaultWetCoilCounterFlowStoreStateVariables;
model HexElementLatent
  ...
  extends Buildings.Fluid.HeatExchangers.BaseClasses.PartialHexElement(
   ...
   redeclare final Buildings.Fluid.MixingVolumes.BaseClasses.MixingVolumeHeatMoisturePort vol2(
     ...
     prescribedHeatFlowRate=false),
    mas(T(start=iniTmas), der_T(start=inider_Tmas)));
  parameter Real iniTmas "Temperature of element";
  parameter Real inider_Tmas
    "Time derivative of temperature (= der(T))";

  FaultInjection.Equipment.ParameterFaults.BaseClasses.FaultWetCoilCounterFlowStoreStateVariables.MassExchange
    masExc(
    redeclare final package Medium = Medium2,
    inip_w=inip_w,
    inix_w=inix_w) "Model for mass exchange"
    annotation (Placement(transformation(extent={{50,-40},{70,-20}})));

  parameter Modelica.Blocks.Interfaces.RealInput inip_w "Water vapor pressure";
  parameter Modelica.SIunits.MassFraction inix_w
