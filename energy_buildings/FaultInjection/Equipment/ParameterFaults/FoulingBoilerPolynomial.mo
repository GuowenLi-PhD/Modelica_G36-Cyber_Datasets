...
model FoulingBoilerPolynomial
  ...
  // Assumptions
  parameter FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves effCur=FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.Constant
    "Curve used to compute the efficiency";
  parameter Real a[:] = {0.9} "Coefficients for efficiency curve";

  parameter Real eta_fou = 1 "Efficiency loss coefficient for efficiency curve";
  ...
  Modelica.SIunits.Efficiency eta=
    if effCur ==FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.Constant then
      a[1]*eta_fou
    elseif effCur ==FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.Polynomial then
      Buildings.Utilities.Math.Functions.polynomial(a=a*eta_fou, x=y)
   elseif effCur ==FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.QuadraticLinear then
      Buildings.Utilities.Math.Functions.quadraticLinear(a=aQuaLin*eta_fou, x1=y, x2=T)
   elseif effCur ==FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.BiQuadratic then
      Buildings.Utilities.Math.Functions.biquadratic(a=aBiQua*eta_fou, x1=y, x2=T-273.15)
   else
      0
  "Boiler efficiency";
...
protected
  ...
  parameter Real aBiQua[6] = if size(a, 1) == 6 then a else fill(0, 6)
  "Auxiliary variable for efficiency curve because biquadratic requires exactly 6 elements";
...
initial equation
  if  effCur == FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.QuadraticLinear then
    assert(size(a, 1) == 6,
    "The boiler has the efficiency curve set to 'FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.QuadraticLinear',
    and hence the parameter 'a' must have exactly 6 elements.
    However, only " + String(size(a, 1)) + " elements were provided.");
  end if;
  if  effCur == FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.BiQuadratic then
    assert(size(a, 1) == 6,
    "The boiler has the efficiency curve set to 'FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.BiQuadratic',
    and hence the parameter 'a' must have exactly 6 elements.
    However, only " + String(size(a, 1)) + " elements were provided.");
  end if;
  ...
  elseif effCur ==FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.BiQuadratic then
    // For this efficiency curve, a must have 6 elements.
    eta_nominal = Buildings.Utilities.Math.Functions.biquadratic(
                                                               a=aBiQua, x1=1, x2=T_nominal-273.15);
  else
     eta_nominal = 999;
  end if;
...
<tr>
<td>FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.Constant</td>
<td><i>&eta; = a<sub>1</sub></i></td>
</tr>
<tr>
<td>FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.Polynomial</td>
<td><i>&eta; = a<sub>1</sub> + a<sub>2</sub> y + a<sub>3</sub> y<sup>2</sup> + ...</i></td>
</tr>
<tr>
<td>FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.QuadraticLinear</td>
...
For <code>effCur = FaultInjection.Equipment.ParameterFaults.BaseClasses.EfficiencyCurves.Polynomial</code>,
