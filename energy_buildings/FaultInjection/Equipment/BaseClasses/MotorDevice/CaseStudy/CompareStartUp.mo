...
model CompareStartUp "Compare start up performance"
  ...
  parameter Real p_nominal = (m_flow_nominal/1000)*dp_nominal/0.7;
  ...
  Buildings.Fluid.Sources.Boundary_pT bou4(  redeclare package Medium =
        Medium, nPorts=1)
  ...
  parameter Buildings.Fluid.Movers.Data.Generic per1(
    ...
    speed_rpm_nominal=1800) "Record with performance data" annotation (
      choicesAllMatching=true, Placement(transformation(extent={{40,80},{60,100}})));
  ...
  LoadDevice.SpeedControlled_Nrpm pum2(
    redeclare package Medium = Medium,
    per=per2) annotation (Placement(transformation(extent={{10,-50},{30,-30}})));
  Modelica.Blocks.Sources.RealExpression spe(y=1800)
    annotation (Placement(transformation(extent={{-40,-30},{-20,-10}})));
  parameter Buildings.Fluid.Movers.Data.Generic per2(
    pressure(V_flow=m_flow_nominal/1000*{0,0.41,0.54,0.66,0.77,0.89,1,1.12,1.19},
        dp=dp_nominal*{1.461,1.455,1.407,1.329,1.234,1.126,1.0,0.85,0.731}),
    speed_rpm_nominal=1800,
    motorEfficiency(V_flow={0}, eta={1}))   "Record with performance data"
    annotation (choicesAllMatching=true, Placement(transformation(extent={{40,-20},
            {60,0}})));
equation
  ...
  connect(pro2.y, pum1.Vbs[1, :]) annotation (Line(points={{-99,-40},{-88,
          -40},{-88,-2},{2,-2},{2,52},{8,52}},
                                         color={0,0,127}));
  connect(pro3.y, pum1.Vcs[1, :]) annotation (Line(points={{-99,-80},{-86,
          -80},{-86,-4},{4,-4},{4,48},{8,48}},
                                         color={0,0,127}));
  connect(spe.y,pum2. Nrpm)
    annotation (Line(points={{-19,-20},{20,-20},{20,-28}}, color={0,0,127}));
