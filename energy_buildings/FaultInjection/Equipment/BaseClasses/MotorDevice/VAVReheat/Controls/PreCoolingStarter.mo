...
block PreCoolingStarter "Outputs true when precooling should start"
  ...
  Modelica.Blocks.Logical.LessThreshold greater2(threshold=6*3600)
    annotation (Placement(transformation(extent={{-40,-80},{-20,-60}})));
  Modelica.Blocks.Logical.GreaterThreshold
                                        greater1(threshold=TOutLim)
    annotation (Placement(transformation(extent={{-40,-40},{-20,-20}})));
  Modelica.Blocks.MathBoolean.And and3(nu=3)
    annotation (Placement(transformation(extent={{28,-6},{40,6}})));
  Modelica.Blocks.Routing.RealPassThrough TSet "Temperature setpoint"
...
equation
  ...
  connect(greater2.y, and3.u[3]) annotation (Line(
      ...
      color={255,0,255},
      smooth=Smooth.None));
  connect(TSet.y, greater.u2) annotation (Line(points={{-59,-10},{-50,-10},{-50,
          2},{-42,2}}, color={0,0,127}));
  connect(controlBus.TRooSetCoo, TSet.u) annotation (Line(
      points={{-62,60},{-90,60},{-90,-10},{-82,-10}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
      extent={{-6,3},{-6,3}}));
  connect(controlBus.TRooMax, greater.u1) annotation (Line(
      points={{-62,60},{-50,60},{-50,10},{-42,10}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
