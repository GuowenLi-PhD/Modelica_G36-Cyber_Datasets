...
equation
  ...
  if not checkValve then
    ...
    head = homotopy((N/N_nominal)^2*flowCharacteristic(V_flow_single*N_nominal/Buildings.Utilities.Math.Functions.smoothMax(1E-05,N,1E-06)),
                     N/N_nominal*(flowCharacteristic(0)+delta_head_init*V_flow_single));
    s = 0;
  else
    // Flow characteristics when check valve is open
    head = homotopy(if s > 0 then (N/N_nominal)^2*flowCharacteristic(V_flow_single*N_nominal/Buildings.Utilities.Math.Functions.smoothMax(1E-05,N,1E-06))
  ...
  if use_powerCharacteristic then
    W_single = homotopy((N/N_nominal)^3*(rho/rho_nominal)*powerCharacteristic(V_flow_single*N_nominal/Buildings.Utilities.Math.Functions.smoothMax(1E-05,N,1E-06)),
                        N/N_nominal*V_flow_single/V_flow_single_init*powerCharacteristic(V_flow_single_init));
    eta = dp_pump*V_flow_single/Buildings.Utilities.Math.Functions.smoothMax(1E-05,W_single,1E-06);
  else
    eta = homotopy(efficiencyCharacteristic(V_flow_single*(N_nominal/Buildings.Utilities.Math.Functions.smoothMax(1E-05,N,1E-06))),
