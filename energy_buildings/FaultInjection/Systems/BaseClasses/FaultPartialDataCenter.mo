within FaultInjection.Systems.BaseClasses;
partial model FaultPartialDataCenter
 ...
 // WSE parameters
  ...
  Buildings.Fluid.HeatExchangers.CoolingTowers.YorkCalc cooTow[numChi](
    ...
    annotation (Placement(transformation(extent={{10,-10},{-10,10}},
      origin={10,140})));
  Equipment.VariableFaults.FaultTemperatureSensor TCHWSup(
    redeclare replaceable package Medium = MediumW,
    m_flow_nominal=numChi*m2_flow_chi_nominal,
    faultMode=faultMode) "Chilled water supply temperature"
  ...
  FaultInjection.Equipment.ParameterFaults.FaultCoolingCoilHumidifyingHeating ahu(
    redeclare replaceable package Medium1 = MediumW,
  ...
  Buildings.Fluid.Sources.Boundary_pT expVesChi(
    ...
    annotation (Placement(transformation(extent={{-10,-10},{10,10}},
        rotation=180,
        origin={132,-113})));
  Equipment.VariableFaults.FaultRelativePressureSensor
                                           senRelPre(
    redeclare replaceable package Medium =MediumW, faultMode=faultMode)
  ...
  Modelica.Blocks.Sources.IntegerExpression chiNumOn(
    y=integer(sum(chiStaCon.y))) "The number of running chillers"
  ...
  Utilities.InsertionTypes.Connectors.PressureDrop res(
    redeclare package Medium = MediumA,
    m_flow_nominal=mAir_flow_nominal,
    dp_nominal=100)                                    annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=-90,
        origin={36,-158})));
 parameter Utilities.InsertionTypes.Generic.faultMode faultMode(startTime=
        startTime, endTime=endTime)
    annotation (Placement(transformation(extent={{108,174},{128,194}})));
  parameter Modelica.SIunits.Time startTime=0 "Fault start time";
  parameter Modelica.SIunits.Time endTime=86400 "Fault end time";
equation
  ...
  connect(roo.airPorts[1], TAirSup.port_b)
    annotation (Line(
      points={{1.525,-188.7},{1.525,-196},{-50,-196},{-50,-160}},
      color={0,127,255},
      thickness=0.5));
  ...
  connect(gai1.u, senRelPre.p_rel)
    annotation (Line(points={{-198,-60},{40,-60},{40,-110},{8,-110},{8,-85}},
                                                          color={0,0,127}));
  connect(ahu.port_a2, res.port_a) annotation (Line(
      points={{20,-126},{36,-126},{36,-148}},
      color={0,127,255},
      thickness=0.5));
  connect(roo.airPorts[2], res.port_b) annotation (Line(
      points={{5.575,-188.7},{5.575,-196},{36,-196},{36,-168}},
      color={0,127,255},
      thickness=0.5));
