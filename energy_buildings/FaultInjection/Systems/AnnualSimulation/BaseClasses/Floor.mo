...
model Floor "Model of a floor of the building"
  ...
  parameter Buildings.HeatTransfer.Data.OpaqueConstructions.Generic conExtWal(final
      nLay=4, material={matWoo,matGyp,matIns,matGyp}) "Exterior construction"
  ...
  Experimental.VAVReheat.ThermalZones.RoomLeakage leaSou(
    redeclare package Medium = Medium,
    VRoo=568.77,
    s=49.91/33.27,
    azi=Buildings.Types.Azimuth.S,
  ...
  Experimental.VAVReheat.ThermalZones.RoomLeakage leaEas(
    redeclare package Medium = Medium,
    VRoo=360.0785,
    s=33.27/49.91,
    azi=Buildings.Types.Azimuth.E,
  ...
  Experimental.VAVReheat.ThermalZones.RoomLeakage leaNor(
    redeclare package Medium = Medium,
    VRoo=568.77,
    s=49.91/33.27,
    azi=Buildings.Types.Azimuth.N,
  ...
  Experimental.VAVReheat.ThermalZones.RoomLeakage leaWes(
    redeclare package Medium = Medium,
    VRoo=360.0785,
    s=33.27/49.91,
    azi=Buildings.Types.Azimuth.W,
  ...
  Equipment.VariableFaults.FaultTemperatureSensorHeatPort temAirSou(faultMode=
        faulttemAirSou, use_uFau_in=false) "South air temperature sensor fault"
    annotation (Placement(transformation(extent={{290,340},{310,360}})));
  Equipment.VariableFaults.FaultTemperatureSensorHeatPort temAirEas(faultMode=
        faulttemAirEas, use_uFau_in=false)
 ...
 parameter Utilities.InsertionTypes.Generic.faultMode faulttemAirEas
    "East zone air temperature sensor fault"
    annotation (Placement(transformation(extent={{312,380},{332,400}})));
 parameter Utilities.InsertionTypes.Generic.faultMode faulttemAirSou
    "South zone air temperature sensor fault"
    annotation (Placement(transformation(extent={{340,380},{360,400}})));
  Modelica.Blocks.Logical.Switch holSwi
    annotation (Placement(transformation(extent={{-116,146},{-96,166}})));
  Modelica.Blocks.Sources.RealExpression realExpression(y=0.05)
    annotation (Placement(transformation(extent={{-152,166},{-132,186}})));
  Modelica.Blocks.Interfaces.BooleanInput nonWorDaySig
    "Unoccupied day signal, true: weekends and holidays, false: working days"
    annotation (Placement(transformation(extent={{-200,136},{-160,176}})));
equation
  ...
  connect(sou.heaPorAir, temAirSou.port) annotation (Line(
      points={{163,-24},{224,-24},{224,100},{264,100},{264,350},{289,350}},
      color={191,0,0},
      smooth=Smooth.None));
  connect(eas.heaPorAir, temAirEas.port) annotation (Line(
      points={{323,76},{286,76},{286,320},{291,320}},
      color={191,0,0},
      smooth=Smooth.None));
  ...
  connect(gaiIntSou.y, sou.qGai_flow) annotation (Line(
      ...
      color={0,0,127},
      pattern=LinePattern.Dash));
  connect(realExpression.y, holSwi.u1) annotation (Line(points={{-131,176},{
          -124,176},{-124,164},{-118,164}}, color={0,0,127}));
  connect(intGaiFra.y[1], holSwi.u3) annotation (Line(points={{-119,110},{-114,
          110},{-114,136},{-130,136},{-130,148},{-118,148}}, color={0,0,127}));
  connect(holSwi.y, gai.u[1]) annotation (Line(points={{-95,156},{-86,156},{-86,
          130},{-108,130},{-108,110},{-102,110}}, color={0,0,127}));
  connect(nonWorDaySig, holSwi.u2)
    annotation (Line(points={{-180,156},{-118,156}}, color={255,0,255}));
