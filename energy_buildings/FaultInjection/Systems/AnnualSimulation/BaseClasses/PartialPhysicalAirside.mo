...
//    "Nominal capacity of heating coil";
  ...
  // sizing parameter
  parameter Modelica.SIunits.HeatFlowRate designCoolLoad = -1.2*m_flow_nominal*1000*15 "Design cooling load";
  parameter Modelica.SIunits.HeatFlowRate designHeatLoad = 0.3*m_flow_nominal*1006*(18 - 8) "Design heating load";
...
//    annotation (Placement(transformation(extent={{118,-36},{98,-56}})));
  ...
  FaultInjection.Equipment.ParameterFaults.FoulingDryCoilEffectivenessNTU heaCoi(
    redeclare package Medium1 = MediumW,
    redeclare package Medium2 = MediumA,
  ...
  Equipment.ParameterFaults.FoulingWetCoilCounterFlow cooCoi(
  ...
  Equipment.VariableFaults.FaultTemperatureSensor TSup(
    redeclare package Medium = MediumA,
    m_flow_nominal=m_flow_nominal,
    allowFlowReversal=allowFlowReversal,
    faultMode=faultTSup)
    annotation (Placement(transformation(extent={{330,-50},{350,-30}})));
  Equipment.VariableFaults.FaultRelativePressureSensor dpDisSupFan(redeclare
      package Medium =
        MediumA, faultMode=faultdpDisSupFan,
    use_uFau_in=false)
    "Supply fan static discharge pressure sensor fault" annotation (Placement(
        transformation(
        extent={{-10,10},{10,-10}},
        rotation=90,
        origin={320,0})));
  ...
  Equipment.VariableFaults.FaultTemperatureSensor TRet(
    redeclare package Medium = MediumA,
    m_flow_nominal=m_flow_nominal,
    allowFlowReversal=allowFlowReversal,
    faultMode=faultTRet) "Return air temperature sensor"
    annotation (Placement(transformation(extent={{110,130},{90,150}})));
  Equipment.VariableFaults.FaultTemperatureSensor TMix(
    redeclare package Medium = MediumA,
    m_flow_nominal=m_flow_nominal,
    allowFlowReversal=allowFlowReversal,
    faultMode=faultTMix) "Mixed air temperature sensor"
    annotation (Placement(transformation(extent={{30,-50},{50,-30}})));
  Equipment.VariableFaults.FaultFlowSensor VOut1(redeclare package Medium =
        MediumA, m_flow_nominal=m_flow_nominal,
    faultMode=faultVOut)                        "Outside air volume flow rate"
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort      TSupSou(
    ...
    m_flow_nominal=mSou_flow_nominal,
    allowFlowReversal=allowFlowReversal)
                            "Discharge air temperature" annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={760,92})));
  Equipment.VariableFaults.FaultTemperatureSensor
                                             TSupEas(
    redeclare package Medium = MediumA,
    initType=Modelica.Blocks.Types.Init.InitialState,
    m_flow_nominal=mEas_flow_nominal,
    allowFlowReversal=allowFlowReversal,
    faultMode=faultTSupEas)              "Discharge air temperature"
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={940,90})));
  ...
  Buildings.Fluid.Sensors.VolumeFlowRate VSupSou_flow(
    ...
    allowFlowReversal=allowFlowReversal) "Discharge air flow rate" annotation (
      Placement(transformation(
        ...
        rotation=90,
        origin={760,130})));
  Equipment.VariableFaults.FaultFlowSensor
                                         VSupEas_flow(
    redeclare package Medium = MediumA,
    initType=Modelica.Blocks.Types.Init.InitialState,
    m_flow_nominal=mEas_flow_nominal,
    allowFlowReversal=allowFlowReversal,
    faultMode=faultVSupEas_flow)         "Discharge air flow rate" annotation (
      Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={940,128})));
  ...
  Buildings.Fluid.Sensors.VolumeFlowRate VSupWes_flow(
    ...
    allowFlowReversal=allowFlowReversal) "Discharge air flow rate" annotation (
      Placement(transformation(
        ...
        rotation=90,
        origin={1300,128})));
  Equipment.VariableFaults.FaultMixingBox                     eco(
    redeclare package Medium = MediumA,
    ...
    dpExh_nominal=10,
    from_dp=false,
    faultMode=faultOADamStu)
                   "Economizer" annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-10,-46})));
...
public
  ...
  parameter Utilities.InsertionTypes.Generic.faultMode faultOADamStu
    "Outdoor air damper stuck fault"
    annotation (Placement(transformation(extent={{-80,-94},{-60,-74}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultSupDucLea
    "Supply air duct leakage fault"
    annotation (Placement(transformation(extent={{460,12},{480,32}})));
  Utilities.InsertionTypes.Connectors.Leakage lea(
    redeclare package Medium = MediumA,
    faultMode=faultSupDucLea,
    m_flow_nominal=m_flow_nominal)
    annotation (Placement(transformation(extent={{480,-26},{500,-6}})));
  Modelica.Blocks.Math.Gain leaFra(k=-0.1*1.2)
    "Fixed leakage mass fraction of the supply duct"
    annotation (Placement(transformation(extent={{440,-26},{460,-6}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultVOut
    "Outside air volume flow rate sensor fault"
    annotation (Placement(transformation(extent={{-120,-16},{-100,4}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultTSup
    "Supply air temperature sensor fault"
    annotation (Placement(transformation(extent={{332,-76},{352,-56}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultTMix
    "Mix air temperature sensor fault"
    annotation (Placement(transformation(extent={{52,-28},{72,-8}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultTRet
    "Return air temperature sensor fault"
    annotation (Placement(transformation(extent={{116,146},{136,166}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultTSupEas
    "South zone discharge air temperature sensor fault"
    annotation (Placement(transformation(extent={{970,80},{990,100}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultVSupEas_flow
    "East zone discharge air flow rate sensor fault"
    annotation (Placement(transformation(extent={{970,120},{990,140}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultdpDisSupFan
    "Supply fan static discharge pressure sensor fault"
    annotation (Placement(transformation(extent={{336,-8},{356,12}})));
  Schedules.Occupany occ
    annotation (Placement(transformation(extent={{-316,-80},{-296,-60}})));
equation
  ...
  connect(TRooAir.u, flo.TRooAir) annotation (Line(
      points={{488,170},{484,170},{484,514},{1168,514},{1168,491.333},{1094.14,
          491.333}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  ...
  connect(heaCoi.port_b2, cooCoi.port_a2) annotation (Line(
      ...
      color={170,213,255},
      thickness=0.5));
  connect(senSupFlo.port_b, lea.port) annotation (Line(
      points={{420,-40},{502,-40},{502,-16},{500,-16}},
      color={170,213,255},
      thickness=0.5));
  connect(leaFra.y, lea.m_flow_leakage)
    annotation (Line(points={{461,-16},{478,-16}}, color={0,0,127}));
  connect(senSupFlo.V_flow, leaFra.u) annotation (Line(points={{410,-29},{410,
          -16},{438,-16}}, color={0,0,127}));
