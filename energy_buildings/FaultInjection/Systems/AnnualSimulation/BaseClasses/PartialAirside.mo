...
partial model PartialAirside
  ...
  extends
    FaultInjection.Systems.AnnualSimulation.BaseClasses.PartialPhysicalAirside(
      heaCoi(dp1_nominal=30000));
  ...
  Buildings.Controls.OBC.ASHRAE.G36_PR1.TerminalUnits.Controller conVAVCor(
    ...
    VOutPerAre_flow=1.25e-4)         "Controller for terminal unit corridor"
  ...
  Buildings.Controls.OBC.ASHRAE.G36_PR1.TerminalUnits.Controller conVAVSou(
    ...
    AFlo=AFloSou,
    final samplePeriod=samplePeriod,
    VOutPerAre_flow=1.25e-4)         "Controller for terminal unit south"
  ...
  Buildings.Controls.OBC.ASHRAE.G36_PR1.TerminalUnits.Controller conVAVEas(
    ...
    AFlo=AFloEas,
    final samplePeriod=samplePeriod,
    VOutPerAre_flow=1.25e-4)         "Controller for terminal unit east"
  ...
  Buildings.Controls.OBC.ASHRAE.G36_PR1.TerminalUnits.Controller conVAVNor(
    ...
    AFlo=AFloNor,
    final samplePeriod=samplePeriod,
    VOutPerAre_flow=1.25e-4)         "Controller for terminal unit north"
  ...
  Buildings.Controls.OBC.ASHRAE.G36_PR1.TerminalUnits.Controller conVAVWes(
    ...
    AFlo=AFloWes,
    final samplePeriod=samplePeriod,
    VOutPerAre_flow=1.25e-4)         "Controller for terminal unit west"
  ...
  Buildings.Controls.OBC.ASHRAE.G36_PR1.AHUs.MultiZone.VAV.Controller conAHU(
    ...
    final peaSysPop=1.2*sum({0.02*AFlo[i] for i in 1:numZon})) "AHU controller"
  ...
  Buildings.Controls.OBC.ASHRAE.G36_PR1.AHUs.MultiZone.VAV.SetPoints.OutdoorAirFlow.Zone
    zonOutAirSet[numZon](
    each VOutPerAre_flow=1.25e-4,
    final AFlo=AFlo,
    final have_occSen=fill(false, numZon),
    final have_winSen=fill(false, numZon),
    each occDen=0.02,
    final desZonPop={0.02*AFlo[i] for i in 1:numZon},
  ...
  Utilities.InsertionTypes.Variables.SignalDelay.RealFixedDelay supFanSpeDel(
      delayTime=1, faultMode=faultSupFanSpeDel) "Supply fan speed delay"
    annotation (Placement(transformation(extent={{260,50},{280,70}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultSupFanSpeDel
    "Return air temperature sensor fault"
    annotation (Placement(transformation(extent={{260,86},{280,106}})));
  Utilities.InsertionTypes.Variables.SignalDelay.RealFixedDelay yCooDel(
      delayTime=1, faultMode=faultyCooDel) "Cooling coil input signal delay"
    annotation (Placement(transformation(extent={{460,472},{480,492}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultyCooDel
    "Cooling coil input signal delay fault"
    annotation (Placement(transformation(extent={{510,474},{530,494}})));
  Utilities.InsertionTypes.Variables.SignalDelay.RealFixedDelay yHeaDel(
      delayTime=1, faultMode=faultyHeaDel) "Heating coil input signal delay"
    annotation (Placement(transformation(extent={{460,520},{480,540}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultyHeaDel
    "Heating coil input signal delay fault"
    annotation (Placement(transformation(extent={{510,524},{530,544}})));
equation
  ...
  connect(TOut.y, conAHU.TOut) annotation (Line(points={{-279,180},{-260,180},
          {-260,625.778},{336,625.778}},
                                   color={0,0,127}));
  connect(dpDisSupFan.p_rel, conAHU.ducStaPre) annotation (Line(points={{309,0},
  ...
  connect(freSta.y, andFreSta.u1) annotation (Line(points={{22,-92},{28,-92},{
          28,-112},{-40,-112},{-40,-130},{-22,-130}},
                                                 color={255,0,255}));
  connect(eco.yOut, conAHU.yOutDamPos) annotation (Line(points={{-10,-34},{
          -10,36},{446,36},{446,522},{424,522},{424,522.667}}, color={0,0,127}));
  connect(supFanSpeDel.u, conAHU.ySupFanSpe) annotation (Line(points={{258,60},
          {240,60},{240,218},{444,218},{444,618.667},{424,618.667}}, color={
          235,0,0}));
  connect(supFanSpeDel.y, fanSup.y) annotation (Line(points={{281,60},{290,60},
          {290,-18},{310,-18},{310,-28}}, color={235,0,0}));
  connect(yCooDel.u, conAHU.yCoo) annotation (Line(points={{458,482},{446,482},
          {446,544},{424,544}}, color={235,0,0}));
  connect(yHeaDel.u, conAHU.yHea) annotation (Line(points={{458,530},{450,530},
          {450,554.667},{424,554.667}}, color={235,0,0}));
  connect(yHeaDel.y, swiFreSta.u3) annotation (Line(points={{481,530},{488,
          530},{488,462},{450,462},{450,34},{22,34},{22,-110},{10,-110},{10,
          -138},{18,-138}}, color={235,0,0}));
  connect(occ.occupied, booRep.u) annotation (Line(points={{-295,-76},{-172,-76},
          {-172,290},{-122,290}}, color={255,0,255}));
  connect(occ.tNexOcc, reaRep.u) annotation (Line(points={{-295,-64},{-188,-64},
          {-188,330},{-122,330}}, color={0,0,127}));
  connect(occ.nonWorDay, flo.nonWorDaySig) annotation (Line(points={{-295,-70},
          {-176,-70},{-176,502},{734,502},{734,489.867},{760.286,489.867}},
        color={255,0,255}));
  annotation (
