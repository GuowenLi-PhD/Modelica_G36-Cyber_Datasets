...
def parse_instances(model_path, file_name, fmu_name = 'fmu', fmi_version = '2.0', fmi_type = 'cs',simulator='jmodelica',  output_directory='.'):
    ...
    fmu_path = translate_fmu(model_path,file_name, fmu_name, fmi_version, fmi_type, simulator,output_directory)
...
def write_wrapper(model_path, file_name, instances,fmi_version='2.0', fmi_type='cs',simulator='jmodelica',outputDirectory='.'):
    """
    ...
    if (len_write_blocks + len_parameter_blocks + len_read_blocks):
        ...
        if file_name:
            wrapped_path = file_name[0]+'\wrapped.mo'
        else:
        ...
        with open(wrapped_path, 'w') as f:
            ...
            for block in instances['OverwriteParameter'].keys():
                ...
                f.write('\tparameter Real {0} = {1} "{2}";\n'.format(_make_var_name(block,style='parameter',attribute='(unit="{0}", min={1}, max={2})'.format(instances['OverwriteParameter'][block]['Unit'], instances['OverwriteParameter'][block]['Minimum'], instances['OverwriteParameter'][block]['Maximum'])), instances['OverwriteParameter'][block]['Start'],instances['OverwriteParameter'][block]['Description']))
                ...
                for ite,item in enumerate(instances_overwrite_ls):
                    for i, block in enumerate(item):
                        if ite == 0: # write parameter
                            ...
                            # need consider nested instances such as X.Y.Z_u -> X(Y(Z(uExt(y={}),activate(y={}))))
                            #if '.' in block:
                            f.write('\t\t{0}(uExt(y={1}),activate(y={2})){3}'.format(block.replace('.','('), input_signals_wo_info[block], input_activate_wo_info[block],')'*block.count('.')))
                            #else: 
            ...
            # Add experiment setting
            f.write('annotation(experiment(Tolerance=1e-06));\n')
            # End file
            f.write('end wrapped;')
        # Export as fmu
        fmu_path = translate_fmu('wrapped',file_name, 'wrapped', fmi_version, fmi_type, simulator,outputDirectory)
    ...
    # If there are not, write and export wrapper model
        ...
        # Compile fmu
        fmu_path = translate_fmu(model_path, file_name, 'wrapped', fmi_version, fmi_type, simulator,outputDirectory)
        wrapped_path = None

    return fmu_path, wrapped_path

def translate_fmu(model_name,file_name, fmu_name, fmi_version = '2.0', fmi_type='cs',simulator='jmodelica',outputDirectory='.'):
    """
    ...
    if simulator is 'jmodelica':
        ...
        if outputDirectory is not '.':
            fmu_name = os.path.join(outputDirectory,fmu_name)
...
def export_fmu(model_path, file_name,fmi_version='2.0', fmi_type='cs',simulator='jmodelica',output_directory='.'):
    ...
    fmu_path, _ = write_wrapper(model_path, file_name, instances, fmi_version, fmi_type, simulator,output_directory)
