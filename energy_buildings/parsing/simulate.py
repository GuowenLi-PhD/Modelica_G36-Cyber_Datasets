...
def simulate(start_time=0, final_time=3600*48, overwrite = None, plot = True):
    ...
    fmu = load_fmu('wrapped.fmu', log_level=2)
    # Get input names
    ...
    # separate into two simulations to see if oveWri_activate could be changed during simulation.
    t = start_time
    overwrite = None
    ...
    while t < final_time:
        ...
        # ncp is the no of communication point. For cs, step = (final-start)/ncp. 
        # ncp has to be a float number instead of integer to avoid premature stop
        options['ncp'] = 500.

        # Simulate
        print(input_object)

        res = fmu.simulate(ts,te, options=options, input=input_object)
