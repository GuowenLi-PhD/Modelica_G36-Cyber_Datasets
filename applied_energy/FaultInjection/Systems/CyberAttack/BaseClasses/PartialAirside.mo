within FaultInjection.Systems.CyberAttack.BaseClasses;
partial model PartialAirside
  "Variable air volume flow system with terminal reheat and five thermal zones"
  extends FaultInjection.Systems.CyberAttack.BaseClasses.PartialPhysicalAirside(
  ...
  Utilities.InsertionTypes.Variables.SignalCorruption.Max maxAtt[5](faultMode=
        faultZoneTemp)
    annotation (Placement(transformation(extent={{1122,500},{1144,520}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultZoneTemp[5](
    each active=false,
  ...
  Utilities.InsertionTypes.Variables.SignalBlock.Blocking fanSpeConAtt(faultMode=
       faultFanSpeed)          "Block the fan speed signal"
    annotation (Placement(transformation(extent={{520,578},{540,598}})));
  parameter Utilities.InsertionTypes.Generic.faultMode faultFanSpeed(
  ...
  Utilities.InsertionTypes.Variables.SignalCorruption.ExternalInteger extAtt(
      faultMode=faultTZoneRequest)
  ...
  parameter Utilities.InsertionTypes.Generic.faultMode faultTZoneRequest(
    each active=false,
    each startTime=207*24*3600 + 12*3600,
    each endTime=207*24*3600 + 12*3600 + 3*3600,
    each minimum=273.15 + 15,
