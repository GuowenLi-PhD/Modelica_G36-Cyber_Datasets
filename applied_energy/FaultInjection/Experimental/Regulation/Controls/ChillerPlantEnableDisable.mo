...
model ChillerPlantEnableDisable
  ...
  Modelica.StateGraph.Transition con2(
    condition=(lesEqu.y >= plaReqTim or TOut < TOutPla - 1 or not ySupFan) and
        onTim.y >= shoCycTim,
    enableTimer=true,
    waitTime=0)
  ...
  .FaultInjection.Experimental.Regulation.Controls.BaseClasses.TimeLessEqual lesEqu(threshold
      =numIgn)
