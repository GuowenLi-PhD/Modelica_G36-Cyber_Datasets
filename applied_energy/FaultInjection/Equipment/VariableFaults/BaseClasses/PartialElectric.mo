within FaultInjection.Equipment.VariableFaults.BaseClasses;
partial model PartialElectric
  ...
  Modelica.Blocks.Sources.RealExpression P_internal(y=P_fault_free)
    "Chiller electric power under fault free condition"
    annotation (Placement(transformation(extent={{0,80},{20,100}})));
  Utilities.InsertionTypes.Variables.SignalCorruption.Scaling sca(faultMode(
      active=chiFauMod.active,
      startTime=chiFauMod.startTime,
      lastForever=chiFauMod.lastForever,
      endTime=chiFauMod.endTime,
      minimum=chiFauMod.minimum,
      maximum=chiFauMod.maximum), use_uFau_in=true)
    "Scaling factor of chiller electric power when the fault is triggered"
    annotation (Placement(transformation(extent={{58,80},{78,100}})));
  ...
  Modelica.Blocks.Sources.RealExpression FIR(y=1 + chiFauMod.fauIntRat*(chiFauMod.perChiFau.c1 +
        chiFauMod.perChiFau.c2*TEvaLvg + chiFauMod.perChiFau.c3*TConEnt + chiFauMod.perChiFau.c4*QEva_flow/
        QEva_flow_nominal)^2 + chiFauMod.perChiFau.c5*QEva_flow/QEva_flow_nominal*
        chiFauMod.fauIntRat + chiFauMod.perChiFau.c6*chiFauMod.fauIntRat)  "The fault impact ratio"
...
protected
  ...
  Modelica.SIunits.Power P_fault_free(nominal=QEva_flow_nominal/COP_nominal,start=QEva_flow_nominal/COP_nominal)
    "Electric power consumed by compressor at fault free condition";
  // Performance data
...
equation
  ...
  if on then
    ...
    // Compressor power.
    P_fault_free = -QEva_flow_ava/COP_nominal*EIRFunT*EIRFunPLR*CR;
  ...
  connect(preHeaFloEva.port, vol2.heatPort) annotation (Line(
      ...
      smooth=Smooth.None));
  connect(sca.y, P)
    annotation (Line(points={{79,90},{110,90}}, color={235,0,0}));
  connect(P_internal.y, sca.u)
    annotation (Line(points={{21,90},{56,90}}, color={0,0,127}));
  connect(FIR.y, sca.uFau_in) annotation (Line(points={{21,24},{40,24},{40,96},{
          56,96}}, color={0,0,127}));
