...
model ModeSelectorMPC
  ...
  State unOccOff(
    mode=MotorDevice.VAVReheat.Controls.OperationModes.unoccupiedOff,
    nIn=3,
    nOut=4) "Unoccupied off mode, no coil protection"
    annotation (Placement(transformation(extent={{-20,20},{0,40}})));
  State unOccNigSetBac(
    nOut=2,
    mode=MotorDevice.VAVReheat.Controls.OperationModes.unoccupiedNightSetBack,
  ...
  State morWarUp(mode=MotorDevice.VAVReheat.Controls.OperationModes.unoccupiedWarmUp,
                                                                            nIn=2,
  ...
  Modelica.Blocks.Logical.LessEqualThreshold occThrSho(threshold=6*3600)
    "Signal to allow transition into morning warmup"
    annotation (Placement(transformation(extent={{-140,-190},{-120,-170}})));
  Modelica.StateGraph.TransitionWithSignal t5(enableTimer=true, waitTime=60)
    annotation (Placement(transformation(extent={{118,20},{138,40}})));
  State occ(       mode=MotorDevice.VAVReheat.Controls.OperationModes.occupied,
                                                                      nIn=3)
    "Occupied mode"
  ...
  State morPreCoo(                                                        nIn=2,
    mode=MotorDevice.VAVReheat.Controls.OperationModes.unoccupiedPreCool,
    nOut=1) "Pre-cooling mode"
    annotation (Placement(transformation(extent={{-40,-140},{-20,-120}})));
  Modelica.StateGraph.Transition t7(condition=TRooMax.y - delTRooOnOff/2 <
        TSetCoo.y or occupied.y)
  ...
  Modelica.Blocks.Sources.BooleanExpression booleanExpression(y=TRooMax.y <
        TSetCoo.y)
  ...
  Modelica.Blocks.Routing.RealPassThrough TRooMax "Maximum room temperature"
    annotation (Placement(transformation(extent={{-80,200},{-60,220}})));
  Modelica.Blocks.Routing.RealPassThrough TSetCoo "Temperature setpoint"
...
equation
  ...
  connect(cb.TRooMax, TRooMax.u) annotation (Line(
      points={{-158,140},{-92,140},{-92,210},{-82,210}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
      extent={{-6,3},{-6,3}}));
  connect(cb.TRooSetCoo, TSetCoo.u) annotation (Line(
      points={{-158,140},{-160,140},{-160,170},{-142,170}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
