within FaultInjection.Equipment.BaseClasses.MotorDevice.VAV;
model ChillerDXHeatingEconomizer
  ...
  parameter Modelica.SIunits.PressureDifference dpAir_nominal(displayUnit="Pa") = 875
  ...
  FaultInjection.Equipment.BaseClasses.MotorDevice.SimpleElectricPump
    fanSup(
    allowFlowReversal=false,
    redeclare package Medium = MediumA,
    per=perFan) "Supply fan"
  ...
  Buildings.Fluid.HeatExchangers.DryCoilEffectivenessNTU cooCoi(
    ...
    show_T=true,
    T_a1_nominal=279.15,
    T_a2_nominal=298.15) "Cooling coil"
  ...
  Buildings.Obsolete.Fluid.Sources.FixedBoundary bouPreChi(redeclare package
              Medium =
               MediumW, nPorts=1)
  ...
  parameter Buildings.Fluid.Movers.Data.Generic perFan(
    ...
    pressure(V_flow=mAir_flow_nominal/1.28*{0,0.41,0.54,0.66,0.77,0.89,1,1.12,
          1.19}, dp=dpAir_nominal*{1.461,1.455,1.407,1.329,1.234,1.126,1.0,0.85,
          0.731}))
    "Record with performance data" annotation (choicesAllMatching=true,
      Placement(transformation(extent={{20,142},{40,162}})));
equation
  ...
  connect(chi.port_b1, out.ports[1]) annotation (Line(points={{90,-174},{
          -112,-174},{-112,42.6667},{-120,42.6667}},             color={0,127,255}));
...
equation
  ...
  connect(returnAir, out.ports[3]) annotation (Line(points={{200,-40},{
          -116,-40},{-116,37.3333},{-120,37.3333}},
                                          color={0,127,255}));
  connect(uEco, ideEco.y) annotation (Line(points={{-220,-60},{-148,-60},{-148,70},
          {-90,70},{-90,57}}, color={0,0,127}));
  connect(uFan, fanSup.f_in) annotation (Line(points={{-220,140},{-28.8,140},{
          -28.8,52}}, color={0,0,127}));
  connect(fanSup.V_rms, V_rms) annotation (Line(points={{-19.6,52},{-19.6,110},
          {-220,110}}, color={0,0,127}));
  connect(fanSup.P, PFan) annotation (Line(points={{-9,48},{0,48},{0,140},{210,
          140}}, color={0,0,127}));
